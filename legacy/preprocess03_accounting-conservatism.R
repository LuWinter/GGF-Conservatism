...
merged_for_khan <- identifier %>% 
  ...
  mutate(
    ...
    Size = log(资产总计),
    Lev = 负债合计 / 资产总计,
    MB = MarketValue / (资产总计 - 负债合计) * 1000
  ) %>% 
  select(
    Stkcd, Year, EPS, Ret, adjRet, DR, adjDR,
...
# 2.1 计算公式
formula_basu <- formula(
  EPS / YearOpen ~ DR + Ret + DR:Ret
)
formula_khan <- formula(
  EPS / YearOpen ~ DR + Ret + Size:Ret + MB:Ret + Lev:Ret + 
    DR:Ret + DR:Size:Ret + DR:MB:Ret + DR:Lev:Ret
)
formula_khan_adj <- formula(
  EPS / YearOpen ~ adjDR + adjRet + Size:adjRet + MB:adjRet + 
    Lev:adjRet + adjDR:adjRet + adjDR:Size:adjRet + 
    adjDR:MB:adjRet + adjDR:Lev:adjRet
)
...
accounting_conservatism <- merged_for_khan %>% 
  ...
  mutate(
    G_Score = a1 + a2 * Size + a3 * MB + a4 * Lev,
...
rio::export(
  ...
  file = file.path(output_path, "accounting-conservatism_2022-12-11.dta")
)
dbWriteTable(
  conn = con_sqlite,
