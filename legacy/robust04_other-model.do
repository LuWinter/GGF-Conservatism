...
winsor2 C_Score Accrual NI NI_lag1 CFO Size Lev GDP_p Age MHRatio MB, cuts(1 99) by(Year) trim replace
...
global MB_reg "MB c.MB#DNI_lag1 c.MB#c.NI_lag1 c.MB#DNI_lag1#c.NI_lag1"
...
global MB_reg2 "MB c.MB#DCFO c.MB#c.CFO c.MB#DCFO#c.CFO"
...
eststo high_fe_control:
	quietly reghdfe NI $base_reg $GGF_reg
	$Size_reg $Lev_reg $GDP_reg $MHRatio_reg $Age_reg $MB_reg, 
	absorb($common_fe $high_fe)
    ;
#delimit cr
quietly estadd local control "YES", replace
...
eststo fe_control2:
	...
	$Size_reg2 $Lev_reg2 $GDP_reg2 $MHRatio_reg2, 
	absorb($common_fe)
    ;
#delimit cr
quietly estadd local control "YES", replace
...
eststo high_fe_control2:
	quietly reghdfe Accrual $base_reg2 $GGF_reg2
	$Size_reg2 $Lev_reg2 $GDP_reg2 $MHRatio_reg2 $MB_reg2, 
	absorb($common_fe $high_fe)
    ;
#delimit cr
quietly estadd local control "YES", replace
...
#delimit ;
eststo low_c_score:
	quietly reghdfe C_Score HoldRatio Size Lev MHRatio Age GDP_p,
	absorb($common_fe)
	;
#delimit cr
quietly estadd local fe_industry "YES", replace
...
#delimit ;
esttab low_c_score high_c_score
	using robust04_c_score.rtf,
	replace label nogap star(* 0.10 ** 0.05 *** 0.01) 
